...
public class FlutterLocalNotificationsPlugin
  ...
  private static final int MAX_BIG_PICTURE_DIMENSION = 2048;
  ...
  private static Bitmap decodeResourceSampled(Context context, int resourceId, int maxDimension) {
  ...
  private static Bitmap decodeFileSampled(String path, int maxDimension) {
    BitmapFactory.Options bounds = new BitmapFactory.Options();
    bounds.inJustDecodeBounds = true;
  ...
  private static Bitmap decodeByteArraySampled(byte[] data, int maxDimension) {
    BitmapFactory.Options bounds = new BitmapFactory.Options();
    bounds.inJustDecodeBounds = true;
  ...
  private static Bitmap decodeFlutterAssetSampled(
      Context context, String assetPath, int maxDimension) throws IOException {
    FlutterLoader flutterLoader = FlutterInjector.instance().flutterLoader();
    String lookupKey = flutterLoader.getLookupKeyForAsset(assetPath);
    BitmapFactory.Options bounds = new BitmapFactory.Options();
    bounds.inJustDecodeBounds = true;
    try (AssetFileDescriptor descriptor = context.getAssets().openFd(lookupKey);
  ...
  private static Bitmap getBitmapFromSource(
      Context context, Object data, BitmapSource bitmapSource, int maxDimension) {
    Bitmap bitmap = null;
    if (bitmapSource == BitmapSource.DrawableResource) {
      bitmap =
          decodeResourceSampled(
              context, getDrawableResourceId(context, (String) data), maxDimension);
    } else if (bitmapSource == BitmapSource.FilePath) {
      bitmap = decodeFileSampled((String) data, maxDimension);
    } else if (bitmapSource == BitmapSource.ByteArray) {
      byte[] byteArray = castObjectToByteArray(data);
      bitmap = decodeByteArraySampled(byteArray, maxDimension);
    }
    ...
    switch (iconSource) {
      ...
      case BitmapFilePath:
        icon =
            IconCompat.createWithBitmap(
                decodeFileSampled((String) data, MAX_ICON_DIMENSION));
        break;
      case ContentUri:
      ...
      case FlutterBitmapAsset:
        try {
          icon =
              IconCompat.createWithBitmap(
                  decodeFlutterAssetSampled(context, (String) data, MAX_ICON_DIMENSION));
        } catch (IOException e) {
